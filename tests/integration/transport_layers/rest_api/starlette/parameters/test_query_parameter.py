...
@pytest.fixture(name="query_parameter", scope="module")
def query_parameter_fixture(mock_service: utils.MockService) -> typing.Iterator[None]:
    custom_request = mock_service.custom_request(
        IntParameter,
        (
            mappings.Mapping(
                attribute="int_field",
                source=mappings.Source.query,
                field="int_field",
            ),
        ),
    )
...
@pytest.mark.usefixtures("query_parameter")
def test_nominal(test_client: starlette.testclient.TestClient) -> None:
    response = test_client.get("/unique", params={"int_field": SAMPLE_VALUE})
...
def test_wrong_type(test_client: starlette.testclient.TestClient) -> None:
    response = test_client.get("/unique", params={"int_field": "not an int"})
    assert response.status_code == 400
    body = response.json()
