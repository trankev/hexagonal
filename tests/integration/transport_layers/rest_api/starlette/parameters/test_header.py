...
@pytest.fixture(name="header_parameter", scope="module")
def header_parameter_fixture(mock_service: utils.MockService) -> typing.Iterator[None]:
    custom_request = mock_service.custom_request(
        IntParameter,
        (
            mappings.Mapping(
                attribute="int_field",
                source=mappings.Source.header,
                field="IntField",
            ),
        ),
    )
...
@pytest.mark.usefixtures("header_parameter")
def test_nominal(test_client: starlette.testclient.TestClient) -> None:
    response = test_client.get("/unique", headers={"intfield": f"{SAMPLE_VALUE}"})
...
def test_wrong_type(test_client: starlette.testclient.TestClient) -> None:
    response = test_client.get("/unique", headers={"intfield": "not an int"})
    assert response.status_code == 400
    body = response.json()
