...
class ResourceListingInteractor(interactors.BRBRInteractor[dict, dict]):
    ...
    async def run(self) -> models.Response[dict, dict]:
...
def rest_api_fixture() -> starlette.applications.Starlette:
    tracer = tracing.setup_tracer("spans")
    rest_api = api.API(
        resources=[
            resources.Resource(
                name="resources",
    ...
    routes = routing.create_routes(rest_api, tracer)
    return starlette.applications.Starlette(debug=True, routes=routes)


@pytest.fixture(name="test_client", scope="session")
def test_client_fixture(rest_api) -> starlette.testclient.TestClient:
    return starlette.testclient.TestClient(rest_api)


def test_list_resource(test_client: starlette.testclient.TestClient):
